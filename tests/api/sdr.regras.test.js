...
const { avaliarInteracao, validarEvento, descreverRegras } = require('../../lib/sdr');

// Quarta-feira, 15h (America/Sao_Paulo) => dentro do horario comercial.
const HORARIO_COMERCIAL = new Date('2026-09-23T18:00:00Z');
// Domingo, 15h (America/Sao_Paulo) => fora do horario comercial.
const FORA_DO_HORARIO = new Date('2026-09-27T18:00:00Z');
...
describe('Robo chat — quando responde e quando cala', () => {
  ...
  it('faz uma pergunta de qualificacao por vez, na ordem definida', () => {
    const { robo } = avaliar({ canal: 'whatsapp', texto: 'Quanto custa o Summer Camp?' });

    expect(robo.responder).toBe(true);
    expect(robo.proximaPergunta).toBe('Para qual data voce quer reservar?');
    expect(robo.mensagens).toHaveLength(2);
    expect(robo.followUp.agendar).toBe(true);
    expect(robo.followUp.emMinutos).toBe(30);
  });

  it('nao promete preco fechado nem disponibilidade', () => {
  ...
  it('encerra a cadencia de follow-up apos 3 tentativas', () => {
    const { robo } = avaliar({
      canal: 'whatsapp',
      texto: 'Quanto custa o coaching?',
      conversa: { tentativasFollowUp: 3 }
    });
...
describe('Robo chat — escalonamento para humano', () => {
  ...
  it('fora do horario comercial avisa o lead e adia o prazo da tarefa', () => {
    ...
    expect(robo.mensagens.join(' ')).toMatch(/segunda a sabado/);
...
describe('Validacao de evento e descricao das regras', () => {
  ...
  it('expoe as regras ativas para auditoria', () => {
    ...
    expect(regras.robo.followUpMinutos).toEqual([30, 1440, 4320]);
